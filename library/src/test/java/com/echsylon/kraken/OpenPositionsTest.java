...
public class OpenPositionsTest {
    ...
    public void requestingOpenPositions_shouldReturnParsedPositionObjects() throws Exception {
        atlantis = MockHelper.start("POST", "/0/private/OpenPositions",
                "{'error': [], 'result': {" +
                        "'TRANSACTION-ID': {" +
                        "  'ordertxid': 'ORDER-ID'," +
                        "  'pair': 'XETHZEUR'," +
        ...
        DefaultRequest<Dictionary<Position>> request =
                (DefaultRequest<Dictionary<Position>>) new Kraken("http://localhost:8080", key, secret)
                        .getOpenPositions(null)
                        .enqueue();

        Dictionary<Position> result = request.get(); // Blocks until Kraken delivers
