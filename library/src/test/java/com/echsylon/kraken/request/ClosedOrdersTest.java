...
/**
 * These test cases will test the "closed orders" feature of the Android Kraken
 * SDK.
 * <p>
...
public class ClosedOrdersTest {
    ...
    @Test
    public void requestingClosedOrders_shouldReturnParsedAndUnwrappedOrderObjects() throws Exception {
        atlantis = startMockServer("POST", "/0/private/ClosedOrders",
                "{'error': [], 'result': {" +
                        "'open': {" +
                        "  'ORDER-ID': {" +
                        "    'refid': '1'," +
        ...
        Dictionary<Order> result =
                ((DefaultRequest<Dictionary<Order>>) getKrakenInstance(key, secret)
                        .getClosedOrders()
                        .enqueue())
                        .get(1, SECONDS);
